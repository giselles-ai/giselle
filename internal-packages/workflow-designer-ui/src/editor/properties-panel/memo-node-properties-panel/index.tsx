import type { MemoNode } from "@giselle-ai/data-type";
import { useWorkflowDesigner } from "@giselle-ai/giselle/react";
import { TextEditor } from "@giselle-ai/text-editor/react";
import { useRef } from "react";
import { PropertiesPanelContent, PropertiesPanelRoot } from "../ui";
import { NodePanelHeader } from "../ui/node-panel-header";

export function MemoNodePropertiesPanel({ node }: { node: MemoNode }) {
	const { updateNodeDataContent, updateNodeData, deleteNode } =
		useWorkflowDesigner();

	const containerRef = useRef<HTMLDivElement>(null);

	return (
		<PropertiesPanelRoot>
			<NodePanelHeader
				node={node}
				onChangeName={(name) => updateNodeData(node, { name })}
				docsUrl="https://docs.giselles.ai/en/glossary/memo-node"
				onDelete={() => deleteNode(node.id)}
			/>
			<PropertiesPanelContent>
				<div ref={containerRef} className="flex flex-col min-h-0">
					<TextEditor
						placeholder="Write your memo here..."
						value={node.content.memo}
						onValueChange={(memo) => updateNodeDataContent(node, { memo })}
						showToolbar={false}
						editorClassName="bg-inverse/10 border-none !pt-[4px] !pr-[8px] !pb-[4px] !pl-[12px] rounded-[8px] min-h-[120px]"
					/>
				</div>
			</PropertiesPanelContent>
		</PropertiesPanelRoot>
	);
}
