...
export type NodeStatus = 'idle' | 'running' | 'completed' | 'failed' | 'selected';
...
const ExecutionStateContext = createContext<ExecutionStateContextType | undefined>(undefined);

export const useExecutionState = () => {
  const context = useContext(ExecutionStateContext);
  if (!context) {
    throw new Error('useExecutionState must be used within an ExecutionStateProvider');
  }
  return context;
};
...
export const ExecutionStateProvider: React.FC<ExecutionStateProviderProps> = ({ children }) => {
