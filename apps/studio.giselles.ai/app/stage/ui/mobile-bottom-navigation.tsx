...
export function MobileBottomNavigation({
	...
	return (
		<nav className="md:hidden fixed bottom-0 left-0 right-0 bg-[var(--color-stage-background)] border-t border-white/10 z-20 h-[60px]">
			...
			{showMobileMenu && (
				<>
					{/* biome-ignore lint/a11y/noStaticElementInteractions: Modal backdrop needs click to close */}
					{/* biome-ignore lint/a11y/useKeyWithClickEvents: Modal backdrop, keyboard handled by modal content */}
					<div
						className="md:hidden fixed inset-0 z-50 bg-black/20"
						onClick={() => setShowMobileMenu(false)}
					>
						{/* biome-ignore lint/a11y/noStaticElementInteractions: Prevents event propagation to backdrop */}
						{/* biome-ignore lint/a11y/useKeyWithClickEvents: Only prevents propagation, no keyboard interaction needed */}
						<div
							className="fixed bottom-[60px] left-4 right-4 bg-[var(--color-stage-background)] border border-white/10 rounded-2xl p-6 animate-in slide-in-from-bottom duration-300 shadow-2xl"
