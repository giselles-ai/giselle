              ...
              <div className="relative z-10">
                ...
                >
                  {apps.map((app) => (
                    ...
                    >
                      {/* Icon */}
                      <div className="w-12 h-12 rounded-full bg-ghost-element flex items-center justify-center flex-shrink-0 transition-all group-hover:bg-primary-100/20">
                        <AppIcon className="h-6 w-6 text-text-muted transition-colors group-hover:text-primary-100" />
                      </div>
                      {/* Content */}
      ...
      {!isCarouselView && app && (
        <div className="fixed inset-x-0 bottom-0 md:absolute md:left-0 md:right-0 z-50 animate-in slide-in-from-bottom-full duration-300">
          <div className="relative z-10 rounded-t-2xl shadow-xl focus:outline-none">
            ...
            />
            <div
              className="absolute -z-10 top-0 left-4 right-4 h-px"
              style={{ background: "var(--glass-highlight-bg)" }}
            />
            <div className="absolute -z-10 inset-0 border border-border rounded-t-2xl" />
            <div className="flex items-center justify-between mb-4 px-6 pt-6">
              <div className="flex items-center gap-3">
                {/* App Thumbnail */}
                <div className="w-10 h-10 rounded-full bg-ghost-element flex items-center justify-center flex-shrink-0">
                  <AppIcon className="h-6 w-6 text-text-muted" />
                </div>
                {/* App Title */}
                <div className="flex flex-col">
                  <h3 className="font-sans text-[16px] font-medium tracking-tight text-text truncate">
                    {app.workspaceName || "Untitled"}
                  </h3>
                  {app.label && app.label !== "Manual Trigger" && (
              ...
              <button
                type="button"
                className="rounded-full p-2 text-text-muted opacity-70 hover:opacity-100 hover:bg-ghost-element-hover focus:outline-none transition-all"
              >
                <X className="h-5 w-5" />
