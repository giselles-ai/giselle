...
import { defaultName, GenerationId } from "@giselle-sdk/giselle";
...
async function enrichActWithNavigationData(
	...
	try {
		...
		// Validate generation ID format using GenerationId.safeParse
		const hasValidGenerationId = (step: { generationId?: string } | null) => {
			if (!step || typeof step.generationId !== "string") return false;
			return GenerationId.safeParse(step.generationId).success;
		};
		...
		if (targetStep && hasValidGenerationId(targetStep)) {
			link += `/${targetStep.id}`;
		}
		return {
			...tmpAct,
