import { reconnectIdentityApi } from "@/services/accounts";
...
export async function GET(request: NextRequest) {
	...
	try {
		const url = await reconnectIdentityApi("github", next);
		return NextResponse.redirect(url);
	} catch (error) {
		console.error("Error reconnecting GitHub identity:", error);
		const errorMessage =
